...
<div class="space-y-6">
  ...
  <x-common.component-card title="Hosts" padding="compact">
    <div class="overflow-x-auto rounded-2xl border border-gray-200 dark:border-gray-800">
      <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
        <thead class="bg-gray-50 dark:bg-gray-950/60"><tr><th class="px-4 py-3 text-left text-gray-500">Host</th><th class="px-4 py-3 text-left text-gray-500">Followers</th><th class="px-4 py-3 text-left text-gray-500">Calls</th><th class="px-4 py-3 text-left text-gray-500">Minutes</th><th class="px-4 py-3 text-left text-gray-500">Coins</th><th class="px-4 py-3 text-left text-gray-500">Video Call</th><th class="px-4 py-3 text-left text-gray-500">Room Gifts / PK</th><th class="px-4 py-3 text-left text-gray-500">Agency Earnings</th></tr></thead>
        <tbody class="divide-y divide-gray-200 dark:divide-gray-800">@forelse($hosts as $row)<tr class="bg-white dark:bg-gray-900"><td class="px-4 py-3"><a class="font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-300" href="{{ route('admin.reports.hosts.show', ['host' => $row['host']->id, 'from' => $from->format('Y-m-d'), 'to' => $to->format('Y-m-d')]) }}">{{ $row['host']->user?->name ?? $row['host']->stage_name }}</a><div class="text-xs text-gray-500 dark:text-gray-400">{{ $row['host']->stage_name }}</div></td><td class="px-4 py-3">{{ number_format($row['host']->followers_count ?? 0) }}</td><td class="px-4 py-3">{{ number_format($row['calls']) }}</td><td class="px-4 py-3">{{ number_format($row['minutes']) }}</td><td class="px-4 py-3">{{ number_format($row['coins']) }}</td><td class="px-4 py-3">{{ number_format($row['video_call_minutes']) }} min<div class="text-xs text-gray-500 dark:text-gray-400">{{ number_format($row['video_call_coins']) }} coins</div></td><td class="px-4 py-3">{{ number_format($row['room_gift_coins']) }}<div class="text-xs text-gray-500 dark:text-gray-400">PK {{ number_format($row['pk_gift_coins']) }} · {{ number_format($row['pk_event_count']) }} events</div></td><td class="px-4 py-3">{{ number_format($row['agency_earnings']) }}</td></tr>@empty<tr class="bg-white dark:bg-gray-900"><td colspan="8" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No hosts attached to this agency.</td></tr>@endforelse</tbody>
      </table>
    </div>
  </x-common.component-card>
  ...
  <div class="grid gap-6 xl:grid-cols-2">
    <x-common.component-card title="Weekly Breakdown" padding="compact">
      <div class="overflow-x-auto rounded-2xl border border-gray-200 dark:border-gray-800">
        <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
          <thead class="bg-gray-50 dark:bg-gray-950/60"><tr><th class="px-4 py-3 text-left text-gray-500">Week</th><th class="px-4 py-3 text-left text-gray-500">Calls</th><th class="px-4 py-3 text-left text-gray-500">Minutes</th><th class="px-4 py-3 text-left text-gray-500">Coins</th><th class="px-4 py-3 text-left text-gray-500">Video Call</th><th class="px-4 py-3 text-left text-gray-500">Room Gifts / PK</th><th class="px-4 py-3 text-left text-gray-500">Agency Earnings</th></tr></thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800">@forelse($weeks as $week)<tr class="bg-white dark:bg-gray-900"><td class="px-4 py-3">{{ \Carbon\Carbon::parse($week['week_start'])->format('d M Y') }}</td><td class="px-4 py-3">{{ number_format($week['calls']) }}</td><td class="px-4 py-3">{{ number_format($week['minutes']) }}</td><td class="px-4 py-3">{{ number_format($week['coins']) }}</td><td class="px-4 py-3">{{ number_format($week['video_call_minutes']) }} min<div class="text-xs text-gray-500 dark:text-gray-400">{{ number_format($week['video_call_coins']) }} coins</div></td><td class="px-4 py-3">{{ number_format($week['room_gift_coins']) }}<div class="text-xs text-gray-500 dark:text-gray-400">PK {{ number_format($week['pk_gift_coins']) }} · {{ number_format($week['pk_event_count']) }} events</div></td><td class="px-4 py-3">{{ number_format($week['agency_earnings']) }}</td></tr>@empty<tr class="bg-white dark:bg-gray-900"><td colspan="7" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No weekly data.</td></tr>@endforelse</tbody>
        </table>
      </div>
    </x-common.component-card>
