    ...
    <x-common.component-card title="Recent Calls" padding="compact">
      <div class="overflow-x-auto rounded-2xl border border-gray-200 dark:border-gray-800">
        <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800">@forelse($recentCalls as $call)<tr class="bg-white dark:bg-gray-900"><td class="px-4 py-3">#{{ $call->id }}</td><td class="px-4 py-3">{{ $call->caller?->name }}</td><td class="px-4 py-3"><div>{{ $call->host?->user?->name }}</div><div class="text-xs text-gray-500 dark:text-gray-400">User #{{ $call->host?->user_id ?? '—' }}</div></td><td class="px-4 py-3">{{ ucfirst($call->type) }}</td><td class="px-4 py-3">{{ number_format((int) $call->total_coins_charged) }}</td></tr>@empty<tr class="bg-white dark:bg-gray-900"><td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No recent calls.</td></tr>@endforelse</tbody>
        </table>
      </div>
    </x-common.component-card>
  </div>
